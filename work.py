...
def drawCat(location):
    x,y = location 
    if dispW - x > 100 and dispH - y > 100 and x > 100 and y > 100: 
        roi = frame[y:y+100, x:x+100] #The area of roi, cropping the frame :D
...
while True:
    ...
    cat_locations = colorDetect()
    for location in cat_locations: 
        drawCat(location)

    # #adding cat image to frame, we do this every frame
    # random_x = 100
    # random_y = 200
    # roi = frame[random_y:random_y+100, random_x:random_x+100] #The area of roi, cropping the frame :D
    # img_bg = cv2.bitwise_and(roi, roi, mask=cat_mask_inv)
    # img_fg = cv2.bitwise_and(cat, cat, mask=cat_mask)
    # dst = cv2.add(img_bg, img_fg)
    # frame[random_y:random_y + 100, random_x:random_x + 100] = dst
